...
# ── 4G PDP output file ────────────────────────────────────────────────────────
PDP_OUTPUT_FILE  = os.path.join(PM_DIR, "4G_PDP.csv")
PDP_COUNTER      = "S+PGW create bearer context success ratio"
PDP_NODES        = ["lmb_vcgw01", "llg_vcgw01"]   # matched case-insensitively
PDP_OUTPUT_COLS  = ["Result Time", "Object Name", PDP_COUNTER]
...
def ensure_output_files():
    ...
    if not os.path.exists(PDP_OUTPUT_FILE):
        pd.DataFrame(columns=PDP_OUTPUT_COLS).to_csv(PDP_OUTPUT_FILE, index=False)
        print(f"[INFO] Created {PDP_OUTPUT_FILE}")
...
def process_pdp_folder(folder_path):
    """
    Scan all CSV files in a pmexport folder for the 4G PDP KPI counter.
    Extracts rows for LMB_vCGW01 and LLG_vCGW01, keyed by time + object name.
    Skips rows where the counter value is empty.
    Returns a DataFrame ready to append to 4G_PDP.csv.
    """
    seen_keys = set()
    rows = []
    ...
    for filepath in csv_files:
        try:
            header_df = pd.read_csv(filepath, nrows=0)
            if PDP_COUNTER not in header_df.columns:
                continue
            df = pd.read_csv(filepath, skiprows=[1], on_bad_lines="skip")
        except Exception as e:
            print(f"[WARN] {os.path.basename(filepath)}: {e}")
        ...
        for _, row in df.iterrows():
            ...
            obj_lower = obj.lower()
            if not any(node in obj_lower for node in PDP_NODES):
                continue

            val = row.get(PDP_COUNTER)
    ...
    if not rows:
        return pd.DataFrame(columns=PDP_OUTPUT_COLS)

    df_out = pd.DataFrame(rows, columns=PDP_OUTPUT_COLS)
    df_out = df_out.sort_values("Result Time").reset_index(drop=True)
    return df_out
...
def main():
    ...
    if not pmexport_folders:
        ...
        for folder in pmexport_folders:
            ...
            # USN KPIs — folder-level merge
            pdp_data = process_pdp_folder(folder)
            usn_data = process_usn_folder(folder)
            ...
            if not pdp_data.empty:
                file_exists = os.path.exists(PDP_OUTPUT_FILE) and os.path.getsize(PDP_OUTPUT_FILE) > 0
                pdp_data.to_csv(PDP_OUTPUT_FILE, mode="a", header=not file_exists, index=False)
                print(f"  → 4G PDP: {len(pdp_data)} rows appended")
            else:
                print(f"  → No 4G PDP data in {folder_name}")
            processed_this_run.add(folder_name)
            delete_folder(folder)

...
class CUPSProcessor:
    """
    Handles a single node CPU CSV for CGW/DGW nodes:
      llg_vcgw.csv, llg_vdgw.csv, lmb_vcgw.csv, lmb_vdgw.csv
    Columns: Result Time, Object Name, CPU average usage, CPU max usage, CPU usage
    """
    ...
    def _resample_hourly(self, df):
        """Resample 5-minute data to 60-minute intervals using max."""
        pivot = df.pivot_table(
            index="Result Time",
            columns="VM Name",
            values="CPU max usage",
        ...
        return pivot.resample("60min").max()

    def plot_cpu_usage(self, output_file, title):
        """
        Plot CPU max usage per VM at 60-min intervals.
        Shows yellow box with latest max CPU value.
        """
        df = self.df.copy()
